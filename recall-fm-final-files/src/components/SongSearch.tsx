...
const SongSearch = ({
  ...
  const search = useCallback(async (query: string) => {
    // Song lookup goes through a Supabase function so API details stay out of the client.
    if (query.length < 2) {
      setSuggestions([]);
      return;
    }
  ...
  const handleInputChange = (value: string, field: "song" | "artist") => {
    ...
    // Debounce avoids calling Deezer on every keystroke while the user types.
  ...
  // Close suggestions when the user clicks outside the song search control.
  useEffect(() => {
