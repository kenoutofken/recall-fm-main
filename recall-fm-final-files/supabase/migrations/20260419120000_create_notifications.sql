-- Stores user-facing activity items for likes and follows.
-- source_table/source_id makes each notification idempotent even if data is backfilled.
...
-- Users can only read and manage notifications addressed to themselves.
...
AS $$
BEGIN
  -- Create a notification for the memory owner, but never notify someone for liking their own memory.
...
AS $$
BEGIN
  -- Ignore self-follows defensively, even though the UI does not offer that action.
...
-- Backfill existing likes so older activity appears after this migration is deployed.
INSERT INTO public.notifications (
  user_id,
  actor_id,
  type,
...
-- Backfill existing follows using the same uniqueness guard as the live trigger.
INSERT INTO public.notifications (
  user_id,
  actor_id,
  type,
