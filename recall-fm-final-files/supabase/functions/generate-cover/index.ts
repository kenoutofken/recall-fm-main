...
serve(async (req) => {
  // Edge functions are called from the browser, so CORS preflight is handled before validation.
  ...
  try {
    ...
    // Validate the user's JWT with Supabase before spending image-generation credits.
    ...
    // The prompt asks for an album-cover style image without text so it can sit behind memory cards.
    ...
    // Decode the generated base64 image and upload it to the same bucket as user-uploaded covers.
